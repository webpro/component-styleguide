        ...
        return ehbs
          ...
          .then(function(templates) {
            ...
            app.get('/all', function(req, res) {
              res.render(
                path.resolve(clientDir, 'all'),
                _.assign({}, staticConfig, {
                  layout: false,
            ...
            app.get('*', function(req, res) {
              ...
              var componentType = parts.length > 1 ? parts[0] : rootName,
                ...
                component = _.find(components.all, {
                  type: componentType,
                  name: componentName
                });

              if (component) {
                var dataForFile = util.getDataForFile(
                  path.resolve(componentDir, component.path)
                );
                res.render(
                  component.path,
                  _.assign(
